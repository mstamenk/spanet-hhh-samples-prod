...
C     Continue only if IMODE is 0, 4 or 5
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! g g > u c u~ c~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g g > u d u~ d~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g g > u s u~ s~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g g > c d c~ d~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g g > c s c~ s~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IPROC=IPROC+1  ! g g > d s d~ s~
      PD(IPROC)=G1*G2
      PD(0)=PD(0)+DABS(PD(IPROC))
      IF (IMODE.EQ.4)THEN
        DSIG2 = PD(0)
